...
# Установка единственного доступного голоса (по умолчанию)
...
def clean_comment(comment):
    cleaned_comment = re.sub(r'[^\w\s\u1F600-\u1F64F]', '', comment)  # Исправлено регулярное выражение
...
async def index():
    ...
    if request.method == 'POST':
        if session.get('is_speaking', False):
            ...
            video_url = (await request.form).get('video_url', '')
            logging.info('Пользователь ввел URL видео: %s', video_url)
            session['video_url'] = video_url
            session['min_length'] = int((await request.form).get('min_length', 0))
            video_id = extract_video_id(session['video_url'])
            if video_id:
                start_speaking(video_id, session['min_length'])
            else:
